...
<template>
  <div>
    <label
      :for="id"
      @click="focus"
      class="base-input-number flex items-center w-full bg-grey-50 rounded-[10px]"
      :class="[
        focused || compVal || compVal == '0' ? 'base-input-number-focus' : '',
        focused ? 'base-input-number-border' : '',
        schema?.$error ? 'error' : '',
        small ? 'small' : '',
        medium ? 'medium' : '',
        disabled ? 'disabled' : '',
