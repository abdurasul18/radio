...
import { IComment, ListingService } from "/@src/services/listing";
...
async function handleDelete(item: IComment) {
  ...
  if (isConfirmed) {
    await ListingService.deleteComment(item.id);
    fetchData();
  }
}
</script>
    ...
    <n-spin :show="loading">
      ...
      <div class="mx-auto space-y-6">
        <div v-for="item in list" :key="item.id" class="flex items-start gap-3 group">
          <!-- Avatar -->
          <n-avatar v-if="item.user?.avatar" round :src="$withBaseUrl2(item.user?.avatar)" />
          <n-avatar v-else round :style="{
            color: '#fff',
          ...
          <div class="flex flex-col items-start w-full">
            ...
            <div class="text-right ml-auto text-xs text-gray-400 mt-0.5 font-mono">
