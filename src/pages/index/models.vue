...
let make_id = ref(route.query.make_id ? Number(route.query.make_id) : null);
...
const { list, loading, search, fetchData, total } = useApiServiceAll<IModel>(
...
let currentItem = ref<IModel | null>(null);

async function deleteItem(item: IModel) {
  let isConfirmed = await confirmDelete(modal);
  if (isConfirmed) {
    await ModelsService.delete(item.id!);
    fetchData();
    toastSuccess();
  }
}
...
<template>
  <div>
    <AppTitle>Avtomobil modellari</AppTitle>
